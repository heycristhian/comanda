...
class UserControllerTest extends AbstractControllerTest {
    ...
    void mustReturn201Created_WhenCallingSaveUser() throws Exception {
        ...
        when(saveUser.execute(Mockito.any(UserRequest.class))).thenReturn(user);
        ...
        mockMvc.perform(post(URL_PATH)
                ...
                .andExpect(jsonPath("$.usuario").value(userRequest.username()))
                .andExpect(jsonPath("$.nomeCompleto").value(userRequest.fullname()))
                .andReturn();

        //then:
        verify(saveUser).execute(userRequest);
    ...
    void mustReturn400BadRequest_SendingAnInvalidEmail_WhenCallingSaveUser() throws Exception {
        ...
        mockMvc.perform(post(URL_PATH)
                ...
                .andExpect(jsonPath("$.fields[0].field", equalTo("username")))
    ...
    void mustReturnBadRequest_PasswordInvalid_WhenCallingSaveUser() throws Exception {
        ...
        when(saveUser.execute(any(UserRequest.class))).thenThrow(new PasswordException(errorMsg));
